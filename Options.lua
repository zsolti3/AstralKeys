...
local autoInsertingKeystone = e.CreateCheckBox(contentFrame, L['AUTO_INSERTING_KEYSTONE'])
autoInsertingKeystone:SetPoint('TOPLEFT', expandedTooltip, 'BOTTOMLEFT', 0, -5)
autoInsertingKeystone:SetScript('OnClick', function(self)
	AstralKeysSettings.general.auto_inserting_keystone.isEnabled = not AstralKeysSettings.general.auto_inserting_keystone.isEnabled
	end)

local chatHeader = contentFrame:CreateFontString(nil, 'OVERLAY', 'InterUIBold_Normal')
chatHeader:SetText(L['!keys chat command'])
chatHeader:SetPoint('TOPLEFT', autoInsertingKeystone, 'BOTTOMLEFT', -10, -20)
...
function InitializeOptionSettings()
	...
	autoInsertingKeystone:SetChecked(AstralKeysSettings.general.auto_inserting_keystone.isEnabled)
